...
| default_bucket_count | 3000 | количество бакетов в каждом тире по умолчанию |
...
| pg_enable | true | при выставлении этой переменной в `false` PG-протокол будет выключен (требуется, если кто-то использует плагины и не хочет postgress) |
...
### Пути
| Имя | Значение по умолчанию | Версия picodata  с которой  доступен параметр | Описание |
| ---      | ---      | ---      | ---      |
...
| memtx_dir | /var/lib/picodata | >=26.2 | размещение данных для файлов memtx (snap) |
...
| vinyl_dir | /var/lib/picodata | >=26.2 | размещение данных для файлов vinyl |
| wal_dir | /var/lib/picodata | >=26.2 |размещение данных для файлов WAL (xlog) |
...
| listen_address | `{{ansible_fqdn}}` | Jinja-шаблон для определения адреса сервера. Для IP можно указать `{{ansible_default_ipv4.address}}` или `{{ansible_eth0.ipv4.address}}` |
| pg_address | `{{ansible_fqdn}}` | Jinja-шаблон для определения адреса сервера для подключения по PostgreSQL-протоколу между инстансами. Для IP можно указать `{{ansible_default_ipv4.address}}` или `{{ansible_eth1.ipv4.address}}`. Используется для разделения внутреннего и внешнего трафика по разным сетевым интерфейсам |
и внешнего трафика по разным сетевым интерфейсам |

## Бэкапы
| Имя | Значение по умолчанию | Описание |
| ---      | ---      | ---      |
| backup_dir | /var/lib/picodata/backup | директория для размещения локальных резервных копий |
...
| force_restore | false | восстановление из бэкапа без запроса пользователя |
...
| expel_delay | 5 | | задержка между попытками выполнить `expel` инстанса в секундах, используется с тегом `rebootstrap` |
| expel_retries | 12 | | количество попыток выполнить `expel` инстанса, используется с тегом `rebootstrap` |
...
| force_rebootstrap | false | используется для ребустрапа единственного инстанса репликасета |
| instance | | | имя инстанса, используется с тегом `rebootstrap` |
| ldap | {} | >=26.2 | словарь с настройкой ldap. Не для промышленной эксплуатации! |
...
| sql_file | | | альяс `sql_file_pre_plugins` |
| sql_file_pre_plugins | '' | | файл с набором команд для выполнения после развертывания кластера, но до установки плагинов |
| sql_file_post_plugins | '' | | файл с набором SQL-команд для выполнения после установки плагинов (и развертывания кластера) |
...
| Имя | Значение по умолчанию | Версия picodata  с которой  доступен параметр | Описание |
| ---      | ---      | ---      | ---      |
| bucket_count | 3000 | | количество бакетов в тире. Можно указывать 0, если picodata версии 26.2 и выше |
...
| experimental_sharding_implementation | false | >=26.2 | включает новую реализацию шардинга. Не для промышленной эксплуатации! |
...
| replication_mode | async | >=26.2 | тип репликации: асинхронная или [синхронная](https://docs.picodata.io/picodata/26.2/basics/sync_replication/). Возможные значения: `async`, `sync` |
| systemd_params | {} | | словарь с [настройками для Systemd](systemd.md) |
| vinyl_dir | /var/lib/picodata | >=26.2 | размещение данных для файлов vinyl |
| wal_dir | /var/lib/picodata | >=26.2 |размещение данных для файлов WAL (xlog) |
| wal_mode | write| >=26.2 | определяет, как WAL сбрасывается на диск, позволяя выбирать между производительностью записи и надежностью данных. Возможные значения: `write`, `fsync` |
...
tiers:
  default:
    ...
    bucket_count: 16384
    extra_vars:
      PLUGIN_ADDR: '0.0.0.0:140<INSTANCE_NUM>'
```

В таком случае для первого инстанса переменная `PLUGIN_ADDR` установит значение `0.0.0.0:14000`, для второго `0.0.0.0:14001`, для третьего `0.0.0.0:14002` и т.д.
...
plugins:
  example:
    path: '../files/weather-cache_0.1.0-ubuntu-jammy.tar.gz'
...
> Начиная с версии Picodata 26.1, настройки плагинов можно задавать в списке тиров:

Пример настройки плагина Radix версии 1.0.0
```yaml
plugins:
  example:
    path: '../files/radix_1.0.0-centos_el8.tar.gz'
    config: '../files/radix-config.yml'
    services:
      radix:
        tiers:
          - default:
...
> Внимание! Если плагин использует внешние файлы (например сертификаты), то эти файлы должны находиться на серверах до установки плагина!
...
- <INSTANCE_NUM> - номер инстанса на сервере в двухзначном формате: 01, 02, 03 и т.д.
- <INSTANCE_NUM3> - номер инстанса на сервере в трехзначном формате: 001, 002, 003 и т.д.

> Внимание! Если плагин, для которого используются настройки устанавливается в уже развернутый кластер, то для правильной установки плагина необходимо выполнить тег `deploy` для того, чтобы настройки прописались в конфигурационные файлы инстансов Picodata!
