...
## Требования для установки и сопровождения кластера
...
- учетная запись для Администратора БД (например `dbadm`) должна входить в группу учетной записи для пользователя, указанного в параметре `user` (например `picodata`)
...
- создать каталоги (с вышестоящими подкаталогами)
```
...
```
d /opt/picodata/run 0775 picodata picodata
d /opt/picodata/run/{{ cluster_name }} 0775 picodata picodata
```
