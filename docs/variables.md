...
| iptoro_cert_file | | файл с сертификатом сервера, используется для настройки mTLS внутри кластера |
| iptoro_key_file | | файл с закрытым ключом сервера, используется для настройки mTLS внутри кластера |
| iptoro_ca_file | | файл с доверенным центром сертификации, используется для настройки mTLS внутри кластера |
...
| auth_webui | | авторизация в WebUI, работает с Пикодатой версии 25.4.2 и выше, если версия меньше, то не выставляйте эту переменную |
