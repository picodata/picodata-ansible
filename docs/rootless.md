...
> **Внимание!** При запуске в `rootless` варианте, приоритет имеют лимиты указанные в файле `/etc/security`, а не те что указаны в юните! Поэтому, при необходимости, Администратор ОС должен изменить эти лимиты на всю систему!
...
> Внимание! В `supervisord` отсутствует понятие альяс для службы, поэтому в этом варианте неудобно работать с именами инстансов, сгенерированными автоматически!
...
- отключить автогенерацию имен инстансов `autonames: false`
