...
> Внимание! Восстанавливаться из резервной копии нужно только той версией ansible-роли, которой создавался бэкап, иначе возможен конфликт с именами каталогов с данными инстансов!
...
---
Пример восстановления кластера при смене сетевой адресации (например на других сервеерах).
Создать файл `ip_replacements.yml`, например, в директории с плейбуком. Файл должен содержать массив замен IP-адресов в формате YAML:

**Пример содержимого ip_replacements.yml:**
```yaml
pd_restore_ip_replacements:
  - old_ip: "СТАРЫЙ_IP_АДРЕС"
    new_ip: "НОВЫЙ_IP_АДРЕС"
  - old_ip: "СТАРЫЙ_IP_АДРЕС"
    new_ip: "НОВЫЙ_IP_АДРЕС"
```
`old_ip` - старый IP-адрес - адрес, который использовался в исходной конфигурации кластера. Должен быть указан в кавычках.

`new_ip` - новый IP-адрес - адрес, который будет использоваться в восстановленном кластере. Должен быть указан в кавычках.


Выполнить роль с тегом `restore` и указанием в переменной `ip_replacements_file` имени ранее подготовленного файла:
```bash
ansible-playbook -i hosts.yml picodata.yml -t restore  -e "ip_replacements_file=ip_replacements.yml"
```
...
## Ребутстрап (пересоздание) инсанса

Требуется в случаях, когда репликация сломана и нет возможности отремонтировать вручную

> В версиях picodata до 26.2 работает только на репликасетах с фактором репликации больше 1!

Команда для ребутстрапа инстанса с именем `default_1_2`
```bash
ansible-playbook -i hosts.yml picodata.yml -t rebootstrap -e instance=default_1_2
```

Можно указать peer-адрес через переменную `peer`
```bash
ansible-playbook -i hosts.yml picodata.yml -t rebootstrap -e instance=default_1_2 -e peer=192.168.101.168:3302
```

Начиная с версии picodata 26.2 можно ребутстрапить единственный инстанс в репликасете, при условии, что у него `bucket_count=0`
```bash
ansible-playbook -i hosts.yml picodata.yml -t rebootstrap -e instance=arbiter_1_1 -e force_rebootstrap=true
```

> Внимание! В данный момент есть проблема с ребутстрапом инстанса, с которого инициализировался кластер! Если у вас возникла такая необходимость, то свяжитесь с технической поддержкой для получения обходного решения
