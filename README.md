...
У пользователя, запускающего роль должны быть права на запись в текущий каталог!

Минимальная версия ansible: 2.9

Максимальная версия ansible: 2.18

Максимальная версия python: 3.13

> **Внимание!** При использовании `ansible 2.10`, установленного из репозитория в **Astralinux 1.7**, при работе роли возникает ошибка: `unhashable type: 'list'`. В таком случае необходимо или установить `ansible` через `pip` (`apt-get remove ansible; pip3 install ansible`), или использовать **Astralinux 1.8**
...
Пример инвентарного файла для 3-х серверов, расположенных в 3 группах, имя файла `hosts.yml`
```yml
all:
  vars:
    ...
    tiers:                         # описание тиров
      arbiter:                     # имя тира
        replicaset_count: 3        # количество репликасетов в кластере
        replication_factor: 1      # фактор репликации
        bucket_count: 0            # количество бакетов в тире можно указывать 0, если picodata версии 26.2 и выше
        can_vote: true             # инстансы этого тира принимают участие в голосовании на выборах raft-лидера
        config:
          memtx:
      ...
      default:                     # имя тира
        ...
        bucket_count: 16384        # количество бакетов в тире
        can_vote: false            # инстансы этого тира не принимают участие в голосовании на выборах raft-лидера
        config:
          memtx:
